...
export function validRGBA(rgba: Rgba | string) {
  if (rgba.length > 3) {
...
export function getHexToRgba(hover: Params["hover"]) {
  if (hover) {
    if (validRGBA(hover.color)) {
      return hover.color;
    } else {
      return `rgba(${HEX2RGBA(hover.color).join(",")})`;
    }
  }
}
